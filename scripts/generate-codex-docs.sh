...
def transform(text: str) -> str:
    ...
    text = text.replace("Source `codex-detect.sh` and check `codex_available`", "Source `reviewer-detect.sh` and check `reviewer_available`")
    text = text.replace("Source `codex-config.sh`", "Source `reviewer-config.sh`")
    text = text.replace("source `scripts/codex-gate.sh`", "source `scripts/reviewer-gate.sh`")
    text = text.replace("scripts/codex-review.sh", "scripts/reviewer-review.sh")
    text = text.replace("Codex unavailable — skipping tier boundary review", "Reviewer unavailable — skipping tier boundary review")
    text = text.replace("Codex Review", "Reviewer Review")
    text = text.replace("On-Demand Codex Review", "On-Demand Adversarial Review")
    text = text.replace("On-demand Codex adversarial review of current diff", "On-demand adversarial review of current diff")
    text = text.replace("--codex-model", "--reviewer-model")
