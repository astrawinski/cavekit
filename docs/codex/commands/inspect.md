...
_Generated from `commands/inspect.md`. Upstream command docs are canonical; this file is the Codex-native reading surface._
...
2. Run `"<local Cavekit plugin root>/scripts/bp-config.sh" model reasoning` and treat the result as `REASONING_MODEL`.
3. Run `"<local Cavekit plugin root>/scripts/bp-config.sh" caveman-active inspect` and treat the result as `CAVEMAN_ACTIVE` (true/false).
...
1. **Site/Plan** — find in `context/plans/` or `context/sites/` (match `*site*`, `*plan*`, or `*frontier*`, exclude `*overview*`; apply `--filter` from `<text after the skill invocation>` if set)
