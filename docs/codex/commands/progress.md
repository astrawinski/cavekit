...
_Generated from `commands/progress.md`. Upstream command docs are canonical; this file is the Codex-native reading surface._
...
Look in `context/plans/` then `context/sites/` for `*site*`, `*plan*`, or `*frontier*` files (exclude `*overview*`). If `--filter` is set (parse from `<text after the skill invocation>`), match against it.
