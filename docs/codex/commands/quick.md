...
_Generated from `commands/quick.md`. Upstream command docs are canonical; this file is the Codex-native reading surface._
...
1. Run `"<local Cavekit plugin root>/scripts/bp-config.sh" summary` and print that exact line once.
2. Run `"<local Cavekit plugin root>/scripts/bp-config.sh" model reasoning` and store it as `REASONING_MODEL`.
3. Run `"<local Cavekit plugin root>/scripts/bp-config.sh" model execution` and store it as `EXECUTION_MODEL`.
4. Run `"<local Cavekit plugin root>/scripts/bp-config.sh" model exploration` and store it as `EXPLORATION_MODEL`.
...
Extract from `<text after the skill invocation>`:
...
1. Execute `"<local Cavekit plugin root>/scripts/setup-build.sh"` with any passthrough flags (`--peer-review`, `--max-iterations`)
