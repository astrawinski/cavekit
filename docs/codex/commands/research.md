...
_Generated from `commands/research.md`. Upstream command docs are canonical; this file is the Codex-native reading surface._
...
2. Run `"<local Cavekit plugin root>/scripts/bp-config.sh" model exploration` and treat the result as `EXPLORATION_MODEL`.
3. Run `"<local Cavekit plugin root>/scripts/bp-config.sh" model reasoning` and treat the result as `REASONING_MODEL`.
